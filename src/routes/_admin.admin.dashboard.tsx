...
function Dashboard() {
  ...
  const uid = useId();
  const gaId = `ga-${uid.replace(/:/g, "")}`;
  const grId = `gr-${uid.replace(/:/g, "")}`;
  const sparkSessions = data?.approvalsTrend.map((d) => d.approved) ?? [];
  const sparkRejected = data?.approvalsTrend.map((d) => d.rejected) ?? [];
          ...
          {isLoading || !data ? (
            ...
            <>
              <KpiTile
                ...
                delta={data.kpis.sessionsTodayDelta}
                spark={sparkSessions}
              />
              <KpiTile
                label="Approval rate"
                value={data.kpis.approvalRate}
                delta={data.kpis.approvalRateDelta}
                suffix="%"
                decimals={1}
                spark={sparkSessions}
              />
          ...
          <div className="rounded-2xl border bg-card p-5 shadow-card lg:col-span-2">
            ...
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart
                  data={data?.approvalsTrend ?? []}
                  margin={{ top: 8, right: 8, left: -16, bottom: 0 }}
                >
                  <defs>
                    <linearGradient id={gaId} x1="0" y1="0" x2="0" y2="1">
